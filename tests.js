    ...
    const tests = [
        ...
        },
        {
            ...
            fn: async () => {
                ...
                assert(actualSwiperTransition === expectedSwiperTransition, `Expected: ${expectedSwiperTransition}, but got: ${actualSwiperTransition}`);
            }
        },
        {
            ...
            fn: async () => {
                ...
                console.log('resetRoundElements test - timer transition:', timerBar.style.transition);
                console.log('resetRoundElements test - swiper transition:', swiper.style.transition);
                const actualTimerTransition = normalizeTransition(timerBar.style.transition);
                const actualSwiperTransition = normalizeTransition(swiper.style.transition);
                assert(actualTimerTransition === 'none', `Expected: 'none', but got: ${actualTimerTransition}`);
                assert(timerBar.style.width === '0%', `Expected: '0%', but got: ${timerBar.style.width}`);
                assert(actualSwiperTransition === 'none', `Expected: 'none', but got: ${actualSwiperTransition}`);
                assert(swiper.style.left === '-300%', `Expected: '-300%', but got: ${swiper.style.left}`);
            }
        },
        {
            ...
            fn: async () => {
                ...
                assert(totalValidCharacters === expectedTotal, `Expected: ${expectedTotal}, but got: ${totalValidCharacters}`);
            }
        },
        {
            ...
            fn: async () => {
                resetTestState();
                const initialPoolSize = Math.ceil(Game.calculateTotalValidCharacters() * 0.01);
                console.log('populateCharacterPool test - character pool size:', Game.getCharacterPool().length);
                console.log('populateCharacterPool test - character pool:', Game.getCharacterPool());
                assert(Game.getCharacterPool().length === initialPoolSize, `Expected: ${initialPoolSize}, but got: ${Game.getCharacterPool().length}`);
                const unlockedPercentage = document.getElementById("unlockedPercentage");
                const expectedPercentage = (initialPoolSize / Game.calculateTotalValidCharacters() * 100).toFixed(2);
                assert(unlockedPercentage.textContent.includes(`(${expectedPercentage}%)`), `Expected: (${expectedPercentage}%), but got: ${unlockedPercentage.textContent}`);
            }
        }
    ];
