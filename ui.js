...
export function setupSwiperTransition(duration) {
    ...
    console.log(`Swiper transition setup with duration: ${duration}, swipeDelay: ${swipeDelay}`);
}
...
export function updateUnlockedPercentage(percentage) {
    if (elements && elements.unlockedPercentageElement) {
        elements.unlockedPercentageElement.textContent = `(${percentage.toFixed(2)}%)`;
    } else {
...
export function updateCharacterDisplay(targetCharacters) {
    elements.characterDisplayText.innerHTML = targetCharacters.map(codePoint => String.fromCodePoint(codePoint)).join("&nbsp;");
}

export function createParticles(x, y, isCorrect) {
    const particleCount = randomIntFromInterval(12, 18);
    const container = document.body;
    const baseColor = isCorrect ? getComputedStyle(document.documentElement).getPropertyValue('--color-correct') : getComputedStyle(document.documentElement).getPropertyValue('--color-incorrect');
...
export function updateBackgroundGradient(timescale) {
    ...
    document.documentElement.style.setProperty('--color-border-primary', primaryColor);
    document.documentElement.style.setProperty('--color-border-secondary', secondaryColor);
}
...
export function setupTimescaleControl(handleTimescaleChange) {
    ...
    selectedDiv.textContent = timescaleSelect.options[timescaleSelect.selectedIndex].text;
    ...
    Array.from(timescaleSelect.options).forEach((option, index) => {
        ...
        optionDiv.addEventListener('click', function(e) {
            updateSelectedOption(this, timescaleSelect, selectedDiv, itemsDiv, handleTimescaleChange);
        });
    ...
    itemsDiv.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' || e.key === ' ') {
            ...
            if (focusedOption) {
                updateSelectedOption(focusedOption, timescaleSelect, selectedDiv, itemsDiv, handleTimescaleChange);
            }
...
function updateSelectedOption(clickedOption, select, selectedDiv, itemsDiv, handleTimescaleChange) {
