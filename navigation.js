...
function addArrowKeySupport(state, elements) {
  document.addEventListener("keydown", function (e) {
    if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(e.key)) {
      ...
      const gridCells = [
        ...elements.gameGrid.querySelectorAll(".grid-cell.in-use"),
      ];
...
function focusInitialElement(key, elements, state) {
  switch (key) {
    case "ArrowUp":
      ...
      break;
    case "ArrowDown":
      elements.gameGrid.querySelector(".grid-cell.in-use")?.focus() ||
...
function handleGridNavigation(key, currentElement, gridCells, elements, state) {
  ...
  switch (key) {
    ...
    case "ArrowLeft":
      nextIndex =
        currentIndex % state.cols === 0 ? currentIndex : currentIndex - 1;
      break;
    case "ArrowRight":
      nextIndex =
        (currentIndex + 1) % state.cols === 0 ? currentIndex : currentIndex + 1;
      break;
  }
  ...
  if (nextIndex >= 0 && nextIndex < gridCells.length) {
    ...
    key === "ArrowDown" &&
    currentIndex >= (state.rows - 1) * state.cols
  ) {
...
function handleCharacterDisplayNavigation(key, gridCells, elements, state) {
  ...
  switch (key) {
    case "ArrowUp":
      gridCells[gridCells.length - state.cols + middleColIndex]?.focus() ||
        elements.startGameButton.focus();
      break;
    case "ArrowRight":
      elements.scoreHolder.focus();
