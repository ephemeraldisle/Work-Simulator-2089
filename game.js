...
class Game {
    ...
    initializeGame() {
        ...
        Grid.createAllGridCells(this.elements.gameGrid, CONSTANTS.GRID_MAX_ROWS * CONSTANTS.GRID_MAX_COLS);
    ...
    startRound() {
        ...
        UI.resetRoundElements().then(() => {
            ...
            console.log('Round setup complete with target characters:', targetCharacters);
        });
    }

    calculateBonusPoints() {
        if (!this.state.firstRound) {
            if (this.state.incorrectPicks === 0 && this.state.correctPicks === this.state.matchingPicks) {
                this.updateScore(1);
            }
    ...
    endRound(isEarly = false) {
        ...
        const roundPercentage = ((Date.now() - this.roundStartTime) / this.state.roundDuration) * 100;
    ...
    updateHintText() {
        const shouldEndRoundEarly = UI.updateHintText(this.state.correctPicks, this.state.matchingPicks);
    ...
    adjustCharacterPool(change) {
        if (change > 0 && this.state.availableCharacters.length > 0) {
            ...
            this.state.characterPool = this.state.characterPool.concat(this.state.availableCharacters.splice(0, addCount));
        } else if (change < 0 && this.state.characterPool.length > CONSTANTS.SPECIAL_CHARACTER_COUNT) {
            const removeCount = Math.min(Math.abs(change), this.state.characterPool.length);
            const removedChars = this.state.characterPool.splice(0, removeCount);
            this.state.availableCharacters = this.state.availableCharacters.concat(removedChars);
        }
        this.updateUnlockedPercentage();
    }
    ...
    updateUnlockedPercentage() {
        const percentage = (this.state.characterPool.length / this.totalValidCharacters) * 100;
    ...
    calculateTotalValidCharacters() {
        return CONSTANTS.VALID_RANGES.reduce((total, range) => total + (range[1] - range[0] + 1), 0);
    }
    ...
    populateCharacterPool() {
        ...
        this.state.availableCharacters = shuffleArray(this.state.availableCharacters);
        const initialPoolSize = Math.ceil(this.state.availableCharacters.length * CONSTANTS.CHARACTER_START_UNLOCK_PERCENT);
        this.state.characterPool = this.state.availableCharacters.splice(0, initialPoolSize);
        this.updateUnlockedPercentage();
    }

    prepareCharacterPool() {
        return shuffleArray([...this.state.characterPool]).slice(0, CONSTANTS.SPECIAL_CHARACTER_COUNT);
    }

    setupRound(targetCharacters) {
        this.determineGridDimensions();
        UI.updateCharacterDisplay(targetCharacters);
        Grid.updateGridVisibility(this.state.rows, this.state.cols, this.elements.gameGrid);
        this.state.matchingPicks = Grid.populateGridCells(targetCharacters, this.state.characterPool, this.elements.gameGrid);
        UI.calculateFontColors(this.state.characterPool.length, this.totalValidCharacters);
    ...
    calculateRoundDuration() {
        const unlockPercentage = this.state.characterPool.length / this.totalValidCharacters;
        const baseDuration = CONSTANTS.INITIAL_ROUND_DURATION -
            (unlockPercentage * (CONSTANTS.INITIAL_ROUND_DURATION - CONSTANTS.MIN_ROUND_DURATION));
        return Math.max(CONSTANTS.MIN_ROUND_DURATION, baseDuration) * this.state.timescale;
    }
    ...
    determineGridDimensions() {
        if (this.state.score < CONSTANTS.MAX_SCORE_FOR_SQUARE_GRID) {
            ...
            this.state.cols = Math.min(CONSTANTS.GRID_MAX_COLS, Math.floor(this.state.score / CONSTANTS.GRID_MAX_ROWS) + 1);
        }
    }
        ...
        this.elements.gameGrid.addEventListener('keydown', (event) => {
            if (event.key === 'Enter' || event.key === ' ') {
                ...
                if (focusedCell && focusedCell.classList.contains('grid-cell')) {
                    ...
                    this.gridClick(Array.from(this.elements.gameGrid.children).indexOf(focusedCell), { clientX: x, clientY: y });
                }
            }
        });
    }
